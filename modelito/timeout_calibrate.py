...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--url", default="http://127.0.0.1", help="Ollama base URL when executing"
    )
    ...
    if args.execute:
        ...
        for i in range(iterations):
            for prompt in prompts:
                ...
                payload = {
                    "model": args.model,
                    "prompt": prompt.replace("{text}", sample_text),
                ...
                code, body = _post_json(
                    endpoint,
                    payload,
                    timeout=int(calib.get("timeout_seconds_per_request", 60)),
                )
