...
def test_retry_with_backoff_eventually_succeeds():
    ...
    out = retry_with_backoff(
        ...
        policy=TransportPolicy(
            max_attempts=4, base_delay=0.1, max_delay=1.0, backoff_factor=2.0
        ),
...
def test_normalize_network_error_timeout_and_http():
    timeout_error = normalize_network_error(
        socket.timeout("timed out"), provider="ollama", operation="probe"
    )
    ...
    http_error = normalize_network_error(
        http_exc, provider="ollama", operation="json_get"
    )
...
def test_envelope_helpers_build_consistent_shapes():
    ...
    err = normalize_network_error(
        ConnectionRefusedError("no listener"), provider="ollama", operation="health"
    )
