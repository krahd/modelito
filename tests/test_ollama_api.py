...
def test_api_version_cli_fallback(monkeypatch):
    ...
    def fake_run(*args, **kwargs):
        return subprocess.CompletedProcess(
            args=args, returncode=0, stdout="ollama 0.9.0\n", stderr=""
        )
...
def test_api_ps_cli_fallback(monkeypatch):
    monkeypatch.setattr("modelito.ollama_service.server_is_up", lambda *_a, **_k: False)
    monkeypatch.setattr(
        "modelito.ollama_service.running_model_names", lambda *_a, **_k: ["m1", "m2"]
    )
    ps = api_ps()
    assert ps == ["m1", "m2"]
...
def test_api_tags_http(monkeypatch):
    ...
    monkeypatch.setattr(
        "modelito.ollama_service.json_get", lambda url, timeout=3.0: ["tag1", "tag2"]
    )
    tags = api_tags()
