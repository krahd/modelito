...
def test_install_command_for_current_platform_uses_detected_backend(monkeypatch):
    monkeypatch.setattr(
        "modelito.ollama_service.shutil.which",
...
def test_download_model_progress_tracks_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "modelito.ollama_service.get_ollama_binary", lambda: "/usr/local/bin/ollama"
    )
...
def test_ensure_model_ready_marks_model_ready(monkeypatch):
    ...
    monkeypatch.setattr(
        "modelito.ollama_service.preload_model",
        lambda host, port, model, timeout=120.0: None,
    )
    monkeypatch.setattr(
        "modelito.ollama_service.running_model_names", lambda host: ["llama3.1:8b"]
    )
